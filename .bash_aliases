...
alias python_no_tcmalloc=$(which python)
python() {
    if [[ -f /usr/local/lib/libtcmalloc.so.4 ]]; then
        ...
        echo -e $msg
        LD_PRELOAD=/usr/local/lib/libtcmalloc.so.4 $(which python) "$@"
    elif [[ -f /usr/lib/libtcmalloc.so.4 ]]; then
        msg="WARNING:Using Google's malloc.\nCall python_no_tcmalloc "
        msg=$msg"to use the default python instead.\n"
        echo -e $msg
        LD_PRELOAD=/usr/lib/libtcmalloc.so.4 $(which python) "$@"
    elif [[ -f $HOME/.local/lib/libtcmalloc.so.4 ]]; then
        msg="WARNING:Using Google's malloc.\nCall python_no_tcmalloc "
        msg=$msg"to use the default python instead.\n"
        echo -e $msg
        LD_PRELOAD=$HOME/.local/lib/libtcmalloc.so.4 $(which python) "$@"
    else
        $(which python) "$@"
    fi
}
